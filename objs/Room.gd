...
func do_jiao():
	...
	if me.jingli < costj or me.qi < costq :
		message_vision("[color=red]{name_cn}弯腰到大缸里舀水，结果失足栽到缸里。[/color]".format(me))
		return
	me.jingli = max(0,me.jingli - costj)
	me.qi = max(0,me.qi - costq)
	if me.mark.jiao and me.mark.jiao > 15 + int(rand_range(0,5)) :
		me.mark.wanle = 1
		message_vision("[color=red]菜园管事说：干的不错，好了，你可以到大师兄鲁坤那里去覆命了！[/color]")
		return 1
	message_vision("[color=green]你用瓢从缸里舀起水，在菜地里浇起水来。[/color]")
	add_busy(2.0)
	me.mark.jiao += 1
	if me.force < 20 and rand_range(0,10) < 6 :
		message_vision("[color=yellow]你在浇水中对于内功的用法有些体会[/color]")
