...
func report_status(ob,effective):
	# 比武现场转播 start
	player = users();
	# 比武现场转播 end
	if (effective):
		message_vision("( $N" + eff_status_msg(ob.query("eff_qi") * 100 / ob.query("max_qi")) + " )\n", ob);
		# 比武现场转播 start
		if (environment(ob).query("short") == "擂台"):
			# for (userno = 0; userno < sizeof(player); userno++)
			for userno in range(player.size() :
				if ((player[userno}.query_temp("view_leitai") &&					\
					environment(player[userno}.query("short") == "武道场")			\
					tell_object(player[userno], "( " + ob.name() + eff_status_msg(ob.query("eff_qi") * 100 / ob.query("max_qi")) + " )\n");
		# 比武现场转播 end
	else:
		message_vision("( $N" + status_msg(ob.query("qi") * 100 / ob.query("max_qi")) + " )\n", ob);
		# 比武现场转播 start
		if (environment(ob).query("short") == "擂台"):
			# for (userno = 0; userno < sizeof(player); userno++)
			for userno in range(sizeof(player)) :
				if ((player[userno}.query_temp("view_leitai") &&					\
					environment(player[userno}.query("short") == "武道场")			\
					tell_object(player[userno], "( " + ob.name() + status_msg(ob.query("qi") * 100 / ob.query("max_qi")) + " )\n");
		# 比武现场转播 end

# This function calculates the combined skill/combat_exp power of a certain
# skill. This value is used for A/(A+B) probability use.
func skill_power(ob, skill,usage):
	var status
	var level
	var power
	var dex
...
#
func do_attack(me,victim,weapon,attack_type,msg):
	var my
	var your
	var prepare
	var action
	...
	if (!me.query("env/combatd2") && (userp(me) || !victim.query("env/combatd2"))):
	...
	# 辟邪刺足者无法躲闪
		...
		# 辟邪刺腕者无法招架
			...
			if (!parry_skill || !SKILL_D(parry_skill).query_parry_msg()):
				parry_skill = "parry";
			...
			defense_factor = victim.query_temp("apply/armor");
			...
			# 被打中效果 add by sega
			martial_skill = victim.query_skill_mapped("parry")
			if (martial_skill):
			...
			martial_skill = me.query_skill_mapped(attack_skill)
			if (martial_skill):
				if (SKILL_D(martial_skill).power_point(me) > 0):
					damage *= SKILL_D(martial_skill).power_point(me);
			#
			# 经威力系数修正后的最终伤害力
			...
			if (random(damage) > victim.query_temp("apply/armor") && ((me.is_killing(victim.query("id"))) && 	\
...
func fight(me, victim):
	...
	#	&& (random(100 + me.query_skill("perception")) < 100) )
	#		return;

	# If victim is busy or unconcious, always take the chance to make an attack.
	if (victim.is_busy() || !living(victim) || victim.query_temp("noliving")):
		...
		if (me.is_fighting(victim) && victim.is_fighting(me)):
			if ((!objectp(me.query_temp("weapon")) && sizeof(me.query_skill_prepare()) > 1) || (me.query("double_attack") && !me.query_temp("weapon"))):
				#空手有prepare则双击
	...
	elif: (random(victim.query_cps() * 3) > me.query_cor()):
		me.set_temp("guarding", 0);
		if (!victim.is_fighting(me)):
			victim.fight_ob(me);
		...
		if (environment(me).query("short") == "擂台"):
			# for (userno = 0; userno < sizeof(player); userno++)
			for userno in range(sizeof(player)):
				if ((player[userno}.query_temp("view_leitai") && environment(player[userno}.query("short") == "武道场"):
					tell_object(player[userno], result);
		# 比武现场转播 end
		return;
	else:
...
func start_berserk(me, obj):
	...
	# Is Player capable for a fight?
	# Are we still inthe same room?
	# Are we in a peace room?

	if (me.is_fighting(obj) || !living(me) || me.query_temp("noliving")			\
			|| environment(me) != environment(obj) 								\
			|| environment(me).query("no_fight")  								\
	):
		return;

	shen = 0 - me.query("shen");
	...
	if (!userp(me) || me.query("neili") > (random(shen) + shen) / 10):
		return;
...
func start_hatred(me, obj):
	...
	# Are we capable for a fight?
	# Is Player capable for a fight?
	# Are we still in the sameroom?
	# Are we in a peace room?

	if (me.is_fighting(obj) || !living(me) || me.query_temp("noliving")						\   	
			|| environment(me) != environment(obj) || environment(me).query("no_fight")): 	 	
		return;

	if (me.query_temp("owner") == obj.query("id") || obj.query_temp("owner") == me.query("id")): # be trained
		return;
...
func winner_reward(killer, victim):
	killer.defeated_enemy(victim);

func killer_reward(killer, victim):
	var vmark
	var temp
	var msg = "莫名其妙地死了"
	var quest_master = ""
	...
	if (userp(victim)):
		...
		""" old
			...
			elif:(student_num == 1) temp = "1";
			elif:(student_num == 2) temp = "2";
		...
		victim.add("combat_exp", -victim.query("combat_exp") / 50);
		victim.delete ("vendetta");
		if (victim.query("thief")):
			victim.set("thief", victim.query("thief") / 2);
		if (victim.query("potential") > victim.query("learned_points")):
			victim.add("potential", (victim.query("learned_points") - victim.query("potential")) / 2);
		...
		if (objectp(killer)):
			msg = "被";
			if (objectp(faintby = victim.query_temp("faint_by")) && faintby != killer):
				# msg += sprintf("%s打晕后，被%s%s趁机",faintby.name(1),
				# 			   faintby.name(1) == killer.name(1) ? "另一" + killer.query("unit") : "",
				# 			   killer.name(1));
				msg += sprintf("%s打晕后，被%s%s趁机",faintby.name(1),"另一" + killer.query("unit"),killer.name(1))
			else:
				msg += killer.name(1);
			msg1 = "被";
			msg1 += killer.name(1);
			actions = killer.query("actions");
			match (actions["damage_type"]):
				"擦伤":
				"割伤":
					msg += "砍死了";
					msg1 += "砍死了";
		...
		CHANNEL_D.do_channel(this_object(), "rumor",sprintf("我看到%s" + msg1 + "。真是好惨。", victim.name(1)));
	...
	if ((userp(killer) || killer.query("is_baby")) && killer.query("combat_exp") < victim.query("combat_exp")) :
		killer.add("shen", -victim.query("shen") / 10);
	...
	if (killer.query("family") && killer.query("family/family_name") == victim.query("family/family_name")):
		killer.add("IKS", 1); # 门派内讧次数累加 internal conflict
		if (killer.query("family/generation") > victim.query("family/generation")):
			killer.add("IKS1", 1); # 犯上次数累加

	if (killer.query("party") && killer.query("party/party_name") == victim.query("party/party_name")):
		killer.add("BKS", 1); # 帮会内讧次数累加
		if (killer.query("party/level") < victim.query("party/level")) :
	...
	if (interactive(killer)):
		if ((quest = killer.query("quest/shan")) && killer.query("quest/shan/time") >= time() && 	\
			(!killer.query("quest/shan/lock")) && (!killer.query("quest/shan/finished")) &&			\
			quest["quest"] == victim.query("name")):
			quest_master = "shan";

		if ((quest_master == "") && (quest = killer.query("quest/helian")) &&						\
			killer.query("quest/helian/time") >= time() && (!killer.query("quest/helian/lock")) &&	\
			(!killer.query("quest/helian/finished")) && quest["quest"] == victim.query("name")):
			quest_master = "helian";
		...
		bonus = killer.query("potential");
		bonus = bonus - killer.query("learned_points");
		bonus = bonus + pot;
		#		if( bonus > 300) bonus = 300;
		bonus += killer.query("learned_points");
		killer.set("potential", bonus);
		bonus = killer.query("score");
		bonus += score;
		killer.set("score", bonus);

		killer.add("shen", -victim.query("shen") / 30);
		"""
		bonus = killer.query("shen");
		if(bonus >= 0)
			bonus += score;
		else:
			bonus -= score;
