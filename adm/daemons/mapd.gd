...
# 查看某个地点是否被MAPD所知晓
func been_known(outdoors:String):
        return stringp(map_short[outdoors};

# 获得某个户外地点的中文名字
func query_map_short(outdoors:String):
    string str;
...
# 获得所有户外地点的集合
func query_all_map_zone():
    return keys(map_short);


# 获得某个户外地点的地图
func query_maps(outdoors:String):
    var maps
	var file
	var map_result
	var i
	maps = map_to[outdoors]
    if (undefinedp(maps)):
        return "未有详细地图。\n";
...
# 由于出错的几率相当小，所以姑且不计。
func get_map_line(line:String, x:int, n:int):
    var rs;
...
func mark_map(map:String, name:String):
	# todo 需要重新设计
	pass
#         var lines;
#         string temp;
...
# 玩家纪录传闻
func record_rumor(obs, topic:String, event_ob):
    var ob;
    var title;
	var n
	var i;
	if (objectp(obs)) :
		...
		return 0;
	title = event_ob.query_detail(topic)	
    if (! stringp(title)):
        return 0;
	if (stringp(event_ob)) :
...
# 获得某个交通地点的信息
func query_trans_info(to:String):
    if (! stringp(to)):
        return map_trans;
    return map_trans[to];

# 获得交通连接信息
func query_connect_info():
    return connect_info;
