...
int valid_scborn(object ob)
{
	...
	skl = ob->query_skills();
	if (!mapp(skl))
		return notify_fail("你的武功还没到家，多练练吧。\n");
	sklname = keys(skl);
	for (i = 0; i < sizeof(skl); i++)
		if (skl[sklname[i]] >= needskill)
			count++;
	if (count < 5)
		return notify_fail("你的武功还没到家，多练练吧。\n");
	if (!ob->query("guangming_winner"))
		return notify_fail("你未见识过明教光明圣火阵，没资格转生。\n");
	if (!ob->query("luohan_winner"))
		return notify_fail("你未见识过少林十八罗汉阵，没资格转生。\n");
	if (!ob->query("qixing"))
		return notify_fail("你未见识过全真天罡北斗阵，没资格转生。\n");
	if (!ob->query("KILLER"))
		return notify_fail("你知道大宗师长什么样吗？\n");
	if (ob->query("KILLER") < needskill * (times + 1) * (times + 1) / 10)
		return notify_fail("宗师对你的评价太差了，再努力一把吧。\n");
...
varargs int ssave(object user, string arg, int lvl)
{
	...
	ob = user->query_temp("link_ob");
	if (!file_size(SKILL_D(arg)))
		return 0;

	if (intp(lvl) && lvl > 0)
		lvl = user->query_skill(arg, 1);
	else
		lvl = 10;
...
varargs int save_perform(object user, string skill, string pfm)
{
	...
	if (stringp(pfm))
	{
		if (!file_size(SKILL_D(skill + "/" + pfm)))
			return 0;
		if (!user->query("perform/" + pfm) && !SCBORN_D->valid_perform(user, skill, pfm))
			return 1;
