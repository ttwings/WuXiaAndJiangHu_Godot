...
func create():
	...
	set_attr("comeout_message", "往$d驰去。\n");
	set_attr("comein_message", "小步跑了过来，蹄声有节奏地踏击着地面。\n");
	set_attr("fleeout_message", "悲嘶一声，往$d落荒而逃。\n");
	set_attr("fleein_message", "摇摇摆摆地跑了过来，鼻孔里呼呼地喷着白气。\n");
...
func setup_stock(ob):
	...
	if( undefinedp(my["max_jing"]) ):
		if( my["age"] <= 3 ) :
			...
			my["max_jing"] = 50 + (my["age"] - 3) * 20;
		elif( my["age"] <= 30 ) :
			my["max_jing"] = 190 + (my["age"] - 10) * 5;
		else :
			my["max_jing"] = 290 + (my["age"] - 30) * 2;
	
	if( undefinedp(my["max_qi"} ) :
		if( my["age"] <= 5 ) :
			my["max_qi"] = 100;
		elif( my["age"] <= 20 ) :
			my["max_qi"] = 100 + (my["age"] - 5) * 20;
		else :
			my["max_qi"] = 400 + (my["age"] - 20) * 5;
	
	if( undefinedp(my["max_jingli"} ) :
		if( my["age"] <= 3 ) :
			my["max_jingli"] = 150;
		elif( my["age"] <= 20 ) :
			my["max_jingli"] = 150 + (my["age"] - 3) * 40;
		else :
			my["max_jingli"] = 830 + (my["age"] - 20) * 10;
