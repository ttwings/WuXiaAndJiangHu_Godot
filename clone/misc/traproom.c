...
void create()
{
	...
	set("long", HIR @LONG
					这是一口不知是谁挖掘的陷阱。陷阱深达数丈，由下往上望去，只
						能见到桌面般大的一片天空。陷阱周围是松软的土层，不停地往外渗水。 似乎刚刚挖好没多久。 LONG NOR);
...
void init()
{
		...
		!me->query_temp("trap/" + file_name(roomtrap)))
	{
		...
		if (roomtrap->query("limit") >= 1)
			roomtrap->add("limit", -1);
		if (depth / 5 > power)
			me->unconcious();
	}
	else
		call_out("flydown", 1, me);
...
{
	return me->query_skill("dodge") * 5 + me->query_dex() * 4 + me->query_int() * 3 + me->query_kar() * 2 + me->query("qi");
}
