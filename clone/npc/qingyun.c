...
object check_place(string file, string dir)
{
	...
	if (sizeof(shorts) < 2)
		return 0;
	if (member_array(room->query("short"), shorts) >= 0)
		return 0;
	return room;
}
void give_place(object ob)
...
int do_answer(string arg)
{
	...
	if (!arg)
		return 0;
	if (!who->query_temp("pend_answer"))
		return 0;
	who->delete_temp("pend_answer");
	arg = lower_case(arg);
	if (who->query_temp("invite"))
		arg = who->query_temp("prize_answer");
	...
	if (exp > 1000)
		exp = 1000;
	if (pot == 0 || pot > exp * 3 / 10 || reason == "奸细")
		pot = exp * 3 / 10;
...
void give_number(object who)
{
	...
	while ((a == "5" && b == "5" && c == "5" && d == "5") || (a == "6" && b == "6" && c == "6" && d == "6") || (a == "8" && b == "8" && c == "8" && d == "8") || (a == "9" && b == "9" && c == "9" && d == "9") || (a == "f" && b == "u" && c == "c" && d == "k"))
	{
		a = all[random(sizeof(all))];
		b = all[random(sizeof(all))];
		c = all[random(sizeof(all))];
...
string cancel_job()
{
	...
	if (!who->query("quest/kill"))
		return "你没有领任务，有什么好取消的";
