...
int do_pour(string arg)
{
	...
	if (ob->query("max_liquid"))
	{
		...
		f = (
			: call_other, __FILE__, "drink_drug"
			:);
...
int drink_drug(object ob)
{
	this_player()->apply_condition("slumber_drug",
								   (int)this_player()->query_condition("slumber_drug") + (int)ob->query("liquid/slumber_effect"));
	return 0;
}
